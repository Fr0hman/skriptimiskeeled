...
grep $SHARE /etc/samba/smb.conf > /dev/null && echo "Selline share ($SHARE) juba on olemas!" && exit 1
...
/etc/init.d/smbd reload > /dev/null 2>&1 || echo "Reloadi ei toimunud, midagi läks valesti." && exit 1
